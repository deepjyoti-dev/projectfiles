# -*- coding: utf-8 -*-
"""
Created on Sat Jul 24 08:00:57 2021
...
def create_app_ui():
    ...
    main_layout = dbc.Container(
        dbc.Jumbotron(
                [
                    html.H1(id = 'heading', children = project_name, className = 'display-3 mb-4',style = {'font-size':'2.4em','font-weight':'bold','font-family':'Roboto Black Italic'}),
                    dbc.Container(
                        dcc.Loading(
                        dcc.Graph(
                            figure = {'data' : [go.Pie(labels=labels, values=values)],
                                      'layout': go.Layout(height = 300, width = 500, autosize = True)
                                      }
                            )
                        ),
                        className = 'd-flex justify-content-center',style={'background-color':'green','border':'2px solid black'}
                    ),
                    
                    html.Hr(),
                    dbc.Textarea(id = 'textarea', className="mb-3 text-center", placeholder="Enter the Review", value = 'My daughter loves these shoes', style = {'height': '150px','border':'2px solid blue','background-color':'#89ABE3FF'}),
                    html.Hr(),
                    dbc.Container([
                        dcc.Dropdown(
                    id='dropdown',
                    ...
                    style = {'margin-bottom': '30px','color':'red'}
